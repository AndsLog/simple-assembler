        ...
        while((Br=br.readLine())!=null){//資料不為空值進入迴圈
			...
			if(token_num>3){
				...
				if(token[1].equals("START")){//若為START
					...
					check_sym(token[0],bw);
				    check_optab(token[1],token[2],bw);
					continue;
				}
			}
			    ...
			    if(token[0].equals("END")){
					...
					total_to_s=Integer.toHexString(total_leng);//轉成16進位
					loc_to_s=Integer.toHexString(loc);//轉成16進位
					bw.write(line+": "+loc_to_s+" "+token[0]+" "+token[1]);
					bw.newLine();
					break;
				}
					...
					loc_to_s=Integer.toHexString(loc);//轉成16進位
					bw.write(line+": "+loc_to_s+" ");
				    check_optab(token[0],token[1],bw);
					continue;
				}
...
//method
	...
	public static void check_start(String name,String ST,String begin,BufferedWriter bw) throws IOException{//抓出program name 及 開始點
	...
	public static void check_optab(String opcode,String operand,BufferedWriter bw) throws IOException{//判斷 op code
	    BufferedWriter Bw=bw;
	    ...
	    if(!op_res.equals("miss")){
            loc=loc+3;
            op_code=op_res;
            Bw.write(op_code+" "+operand);
        }
        else if(opcode.equals("WORD")){
            loc=loc+3;
            Bw.write(opcode+" "+operand);
	...
	public static void check_sym(String symbol,BufferedWriter bw) throws IOException{//檢查symbol table中有無重複的symbol 及 是否是保留字元
